...
(function() {
    ...
    function findSitePath() {
        var g = $axure.player.getHashStringVar(PLUGIN_VAR_NAME);
        if (g == 11) {
            return "all"
        }
        var shortId = $axure.player.getHashStringVar(PAGE_ID_NAME);
        if (!shortId) {
            return []
        }
        var target = null
        function setPath(node) {
            ...
            if (node.children && node.children.length > 0) {
                for(var i = 0; i < node.children.length; i++) {
                    ...
                    setPath(node.children[i])
                }
            }
        }
        var rootNodes = $axure.document.sitemap.rootNodes;
        rootNodes.forEach(setPath);
    ...
    function generateSitemap() {
        ...
        for(var i = 0; i < rootNodes.length; i++) {
            treeUl += generateNode(rootNodes[i], 0, openNodes);
        }
        treeUl += "</ul></div>";
    ...
    function generateNode(node, level, openNodes) {
        var isOpen = "all" === openNodes || (openNodes.indexOf(node.id) != -1);
        var hasChildren = (node.children && node.children.length > 0);
        var margin, returnVal, linkClass, ulStyle;
        if(hasChildren) {
            margin = (9 + level * 17);
            linkClass = isOpen ? 'sitemapMinus' : 'sitemapPlus';
            returnVal = "<li class='sitemapNode sitemapExpandableNode'><div><div class='sitemapPageLinkContainer' style='margin-left:" + margin + "px'><a class='sitemapPlusMinusLink'><span class='" + linkClass + "'></span></a>";
        } else {
            margin = (19 + level * 17);
        ...
        if(hasChildren) {
            ulStyle = isOpen ? '' : 'display: none';
            returnVal += "<ul style='" + ulStyle + "'>";
            for(var i = 0; i < node.children.length; i++) {
                var child = node.children[i];
                returnVal += generateNode(child, level + 1, openNodes);
            }
            returnVal += "</ul>";
        }
        returnVal += "</li>";
